...
//    [self pullViewInSubview:CGPointMake(newXOrigin, newYOrigin) pangesture:sender];
//  }
//  else {
...
- (void)pullViewInSubview:(CGPoint)point pangesture:(UIPanGestureRecognizer *)pangesture {
  //handle make sure new position always show in subviews
  
  NSInteger tmp = 5;
  CGFloat newXOrigin = point.x;
  CGFloat newYOrigin = point.y;
  {
    if (newXOrigin < tmp) {
      newXOrigin = tmp;
    }
    ...
    if (newXOrigin + CGRectGetWidth(self.frame) + tmp > CGRectGetWidth([self superview].frame)) {
      newXOrigin = CGRectGetWidth([self superview].frame) - CGRectGetWidth(self.frame) - tmp;
    }
    
    if (newYOrigin + CGRectGetHeight(self.frame) + tmp > CGRectGetHeight([self superview].frame)) {
      newYOrigin = CGRectGetHeight([self superview].frame) - tmp - CGRectGetHeight(self.frame);
    }
  }
  
  CGRect frame = CGRectMake(newXOrigin,
                            newYOrigin,
                            CGRectGetWidth(self.frame),
                            CGRectGetHeight(self.frame));
  ...
  [UIView animateWithDuration:0.3f delay:0.f options:UIViewAnimationOptionCurveEaseOut animations:^{
    ...
    // Set
    [userDefaults setObject:NSStringFromCGPoint(pangesture.view.center) forKey:@"td_float_icon_point"];
    // Save
    [userDefaults synchronize];
    
  } completion:^(BOOL finished) {
    
    //    [self createTimeChangeState_Stimulate];
  }];
